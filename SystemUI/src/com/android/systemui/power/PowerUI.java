...
public class PowerUI extends SystemUI {
    ...
    void updateBatteryWarningLevels() {
        ...
        mLowBatteryReminderLevels[0] = 5;
        mLowBatteryReminderLevels[1] = 3;
        mLowBatteryAlertCloseLevel = 11;                
    }
    ...
    private final class Receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
				...
				if (mBatteryLevel <= mLowBatteryReminderLevels[1]) {
					if(!mPlugged){
						shutDown();
					}
            ...
            //AnalogLowBattery(4,3);
        }
    };
    private void AnalogLowBattery(int mOldBatteryLevel,int mNowBatteryLevel){
        final int AoldBatteryLevel = mOldBatteryLevel;
        int AmBatteryLevel = mNowBatteryLevel;
        final int AoldBatteryStatus = mBatteryStatus;
        int AmBatteryStatus = BatteryManager.BATTERY_STATUS_NOT_CHARGING;

        final boolean Aplugged = false;
        mPlugged = Aplugged;
        final boolean AoldPlugged = false;

        int AoldBucket = findBatteryLevelBucket(AoldBatteryLevel);
        int Abucket = findBatteryLevelBucket(AmBatteryLevel);

        if (true) {
            Slog.d(TAG, "buckets   ....." + mLowBatteryAlertCloseLevel
                    + " .. " + mLowBatteryReminderLevels[0]
                    + " .. " + mLowBatteryReminderLevels[1]);
            Slog.d(TAG, "level          " + AoldBatteryLevel + " --> " + AmBatteryLevel);
            Slog.d(TAG, "status         " + AoldBatteryStatus + " --> " + AmBatteryStatus);
            Slog.d(TAG, "bucket         " + AoldBucket + " --> " + Abucket);
            Slog.d(TAG, "plugged        " + AoldPlugged + " --> " + Aplugged);
        }
        ...
        if (!Aplugged
                && (Abucket < AoldBucket || AoldPlugged)
                && mBatteryStatus != BatteryManager.BATTERY_STATUS_UNKNOWN
                && Abucket <= 0) {
            // only play SFX when the dialog comes up or the bucket changes
            final boolean playSound = Abucket != AoldBucket || AoldPlugged;
            mWarnings.showLowBatteryWarning(playSound);
        } else if (Aplugged || (Abucket > AoldBucket && Abucket > 0)) {
            mWarnings.dismissLowBatteryWarning();
        } else {
            mWarnings.updateLowBatteryWarning();
        }
        if (AmBatteryLevel <= mLowBatteryReminderLevels[1]) {
            if(!mPlugged){
                shutDown();
            }
	...
	private void shutDown() {
		Intent intentCloseDevice = new Intent(Intent.ACTION_REQUEST_SHUTDOWN);
		intentCloseDevice.putExtra(Intent.EXTRA_KEY_CONFIRM, false);
		intentCloseDevice.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		mContext.startActivity(intentCloseDevice);
	}
}
